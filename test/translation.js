...
const verify = (options) => {
  ...
  const transaction = txn(state);
  const expectedTransaction = txnFromOps ? txnFromOps(state) : transaction;
  ...
  describe('string insert', () => {
    describe('single character insert from position 0', () => {
      verify({
        before: '',
        after: 'd',
        txn: (editorState) => transaction.change(new ChangeDesc(0, 0, ['d'])),
        ops: [{ p: [0], si: 'd' }],
      });
    });
    ...
    //    before: 'HelloWorld',
    //    after: 'Hello World',
    //    txn: (transaction) => transaction.change(new ChangeDesc(5, 5, [' '])),
    //    ops: [{ p: [5], si: ' ' }],
    //  });
    //});
    ...
    //    txn: (transaction) => transaction.change(new ChangeDesc(10, 10, ['Peace'])),
    //    ops: [{ p: [10], si: 'Peace' }],
    //  });
    //});
  });
